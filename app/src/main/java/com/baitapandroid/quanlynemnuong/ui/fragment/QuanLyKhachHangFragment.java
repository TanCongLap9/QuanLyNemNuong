...
public class QuanLyKhachHangFragment extends Fragment {
    ...
    private EditText maKh, hoTen, email, ngaySinh, sdt, taiKhoan, matKhau;
    ...
    public QuanLyKhachHangFragment(KhachHangModel model, boolean insertMode) {
        this.model = model;
    ...
    private void loadViews() {
        hoTen = getView().findViewById(R.id.quan_ly_khach_hang_ho_ten);
        email = getView().findViewById(R.id.quan_ly_khach_hang_email);
        sdt = getView().findViewById(R.id.quan_ly_khach_hang_sdt);
        maKh = getView().findViewById(R.id.quan_ly_khach_hang_ma);
        ngaySinh = getView().findViewById(R.id.quan_ly_khach_hang_ngay_sinh);
        taiKhoan = getView().findViewById(R.id.quan_ly_khach_hang_tai_khoan);
        matKhau = getView().findViewById(R.id.quan_ly_khach_hang_mat_khau);
        nam = getView().findViewById(R.id.quan_ly_khach_hang_nam);
        nu = getView().findViewById(R.id.quan_ly_khach_hang_nu);
        nutSua = getView().findViewById(R.id.quan_ly_khach_hang_sua);

        if (insertMode) maKh.setEnabled(true);
    ...
    private void onNutSuaClick(View view) {
        ...
        boolean emptyPassword = matKhau.getText().toString().isEmpty();
        if (emptyPassword)
            matKhau.setText(matKhauHienTai);
        model = new KhachHangModel(
                Integer.parseInt(maKh.getText().toString()),
    ...
    private void sua() {
        ...
        connection.updateKhachHang(model.getMaKh(), model);
        connection.close();
        Toast.makeText(getContext(), "Sửa khách hàng thành công.", Toast.LENGTH_SHORT).show();
    }
    ...
    public boolean validateInput() {
        ...
        // Check not blank
        for (EditText edt : new EditText[] {maKh, hoTen, ngaySinh, email, sdt, taiKhoan}) {
